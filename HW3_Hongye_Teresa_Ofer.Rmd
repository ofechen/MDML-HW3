...
for (field in na1s) {
 sqf.data$field <- recode.1na(field)
  
}

convert.na <- function(x){
 sqf.data %>% 
    filter(year == "2014") %>% 
    mutate(
      x = recode.1na(x)
...
sqf.data <- map_df(na1s, convert.na)
          
      
proportion.nas <- sqf.data %>% group_by(year) %>% summarize_all(funs(nas =sum(is.na(.))/n()))
proportion.nas <- proportion.nas*100
    
          
d2014 <- sqf.data.full%>% filter(year==2014)
