...
```{r}
# Sourcing library file and importing the data
...
```{r}
# IMPORT RAW DATA
sqf.data.full <- foreach::foreach(year=2008:2016, .combine='rbind') %dopar% {
  filename <- paste0(year, '.csv')
  this.data <- read_csv(filename)
  names(this.data) <- tolower (names(this.data))
  if(length(names(this.data))==111){this.data[,"forceuse"] <- NA}
  this.data
}
```
...
sqf.data <- sqf.data %>% mutate(found.gun = (found.pistol | found.rifle | found.machinegun | found.assault),
                                found.weapon = (found.pistol | found.rifle | found.machinegun| found.assault | found.knife | found.other))
# add a unique id
...
# Change NAs for these columns to False.
na1s <- c("extra.reports","radio.run","force.hands","force.wall","force.ground","force.drawn","force.pointed","force.baton",
          ...
          "stopped.bc.other","found.other","found.pistol","found.rifle","found.assault",
          "found.knife","found.machinegun","additional.report","additional.investigation",
          "additional.proximity","additional.evasive","additional.associating","additional.direction",
          "additional.highcrime","additional.time","additional.sights","additional.other","searched.other",
          "searched.admission","searched.outline","searched.hardobject","frisked.bc.suspected.crime",
          "frisked.bc.weapons","frisked.bc.attire","frisked.bc.actual.crime","frisked.bc.noncompliance",
          "frisked.bc.threats","frisked.bc.prior","frisked.bc.furtive","frisked.bc.bulge",
          "found.gun", "found.weapon")
sqf.data[na1s][is.na(sqf.data[na1s])] <- FALSE
sqf.data[na1s][sqf.data[na1s]==1] <- TRUE

# remove forceuse column as it isn't in prof's csv file.
sqf.data <- sqf.data %>% select(-forceuse)

# observation.period still has more than 10% NAs in years 2012 and 2013. What to do?
# I think we should just zero them out -- Ofer
sqf.data$observation.period[is.na(sqf.data$observation.period)] <- 0
proportion.nas <- sqf.data %>% group_by(year) %>% summarize_all(funs(nas=sum(is.na(.))/n())) * 100
